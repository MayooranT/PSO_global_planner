...
void saveToCSV(const std::string& filename, const std::vector<std::string>& values) {
    ...
    // Write values to the file, separated by commas
    for (size_t i = 0; i < values.size(); ++i) {
        file << values[i];
        if (i < values.size() - 1) { // Avoid trailing comma
            file << ",";
        }
    }
...
namespace global_motion_planner
{
   ...
   */
  PSO::PSO(int nx, int ny, double resolution, double origin_x, double origin_y,int n_particles,int n_inherited,int pointNum , double w_inertial, double w_social, double w_cognitive, double obs_factor ,int max_speed, int initposmode, bool pub_particles,int max_iter)
    : nx_(nx)
    , ny_(ny)
    , ns_(nx*ny)
  ...
  bool PSO::plan(const unsigned char* global_costmap, const std::pair<int, int>& start, const std::pair<int, int>& goal, std::vector< std::pair<int, int>>& path)
  {
    ...
    std::string filename = "/home/mayooran/Documents/ece788/src/pso_global_planner/params/fitness_values.csv";
    saveToCSV(filename , {std::to_string(Best_particle.fitness)});
  ...
  int PSO::grid2Index(int x, int y){return x + nx_ * y;}
